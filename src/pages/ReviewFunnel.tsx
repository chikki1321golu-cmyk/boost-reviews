...
export default function ReviewFunnel({ business }: ReviewFunnelProps) {
  ...
  // Resolved direct review URL — resolved once on mount for all customers
  const [directReviewUrl, setDirectReviewUrl] = useState<string | null>(
    // If place ID already saved, use it instantly (no network call needed)
    business.google_place_id
      ? `https://search.google.com/local/writereview?placeid=${business.google_place_id}`
      : null
  );

  // On mount: if no place ID yet, resolve the URL via edge function
  useEffect(() => {
    if (business.google_place_id) return; // already resolved
    if (!business.google_review_link) return; // nothing to resolve
      ...
      try {
        const { data, error } = await supabase.functions.invoke("resolve-google-place", {
          body: {
            ...
            businessId: business.id, // edge fn will auto-save place_id to DB for next time
          },
        });
        if (error) throw error;
        if (data?.reviewUrl) {
          setDirectReviewUrl(data.reviewUrl);
        }
      } catch (e) {
        console.error("Could not resolve Google review URL:", e);
        // Fallback: use whatever link the customer saved
        setDirectReviewUrl(business.google_review_link);
      }
    };

    resolve();
  }, [business.id, business.google_place_id, business.google_review_link]);
